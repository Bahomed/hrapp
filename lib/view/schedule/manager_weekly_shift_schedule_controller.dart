...
class ManagerWeeklyShiftScheduleController extends GetxController {
  final ShiftScheduleRepository _repo = ShiftScheduleRepository();

  final RxBool isLoading = false.obs;
  final RxString errorMessage = ''.obs;
  final RxList<ManagerShiftEmployee> employees = <ManagerShiftEmployee>[].obs;
  ...
  List<DateTime> get monthDates {
    ...
    return List.generate(daysInMonth, (i) => DateTime(m.year, m.month, i + 1));
  }
  ...
  List<ManagerShiftEmployee> get filteredEmployees {
    final q = searchQuery.value.trim().toLowerCase();
    if (q.isEmpty) return employees;
    return employees
        .where((e) =>
            e.name.toLowerCase().contains(q) ||
            e.employeeNo.toLowerCase().contains(q))
        .toList();
  }
  ...
  DaySchedule? getDaySchedule(ManagerShiftEmployee emp, DateTime date) {
    final dateStr = _fmt(date);
    try {
      return emp.days.firstWhere((d) => d.date == dateStr);
    } catch (_) {
      return null;
    }
  }
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      employees.assignAll(response.employees);
